		...
		if (contractAddress.startsWith("0x") && contractAddress.length == 42) {
			...
			axios(
				...
				.then((res) => {
					...
					interaction.reply({ content: "Traits aren't currently available", ephemeral: true});
				});
		} else {
			let embed = new EmbedBuilder()
				.setColor(0x0099FF)
			...
			axios.get(
				...
				}).catch((err) => interaction.reply({ content: "Collection does not exist. Try again, otherwise use contract address",  ephemeral: true }));
		}
